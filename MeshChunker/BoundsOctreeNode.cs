...
public class BoundsOctreeNode<T>
{
	...
	/// </summary>
	public void DrawAllObjects(Matrix4x4 localToWorld)
	{
		...
		{
			Gizmos.DrawCube(localToWorld.MultiplyPoint(obj.Bounds.center), obj.Bounds.size);
		}
		...
		if (children != null)
		{
			for (int i = 0; i < 8; i++)
			{
				children[i].DrawAllObjects(localToWorld);
			}
		}
